...
if auth_config:
    client = Anthropic(api_key=auth_config['token'], http_client=httpx.Client(verify=False))
                ...
                for media_file in media_files:
                    try:
                        ...
                        if img.mode == 'RGBA':
                            ...
                            img = img.convert('RGB')
                        # Feedback #1: vendor requires black-and-white images.
                        # Convert to grayscale ('L') so BMP output is B&W.
                        img = img.convert('L')
...
class CompleteDOCXExtractor:
        ...
        while i < len(content):
            ...
            while j < len(content):
                ...
                if nxt['type'] == 'table':
                    ...
                    # Only attempt merging for narrow (≤2 col) tables; 3+ col tables
                    # (Q&A / FAQ style) stay separate to preserve TABLE count parity with vendor.
                    if nxt_cols == num_cols and 0 < num_cols <= 2:
    ...
    def _detect_cover_page(self, content: List[Dict]) -> List[Dict]:
        ...
        for item in para_items[:cover_threshold]:
            ...
            if (not _past_title and not para.skip
                ...
                _past_title = True
            # Fix #3 (extended): Skip contact info (phone/fax/tel/postal code)
            # BEFORE main title: PDF headers repeat commission contact details.
            # EXCEPTION: email addresses are NEVER skipped — they are always meaningful
            # document content (contact emails, CSA secretariat, etc.) that the vendor
            # includes in the FREEFORM body even when on their own line.
            _text_no_email = re.sub(
                r'[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}', '', text
            ).strip()
            _is_email_only_line = para.patterns.get('has_email') and len(_text_no_email) <= 8
            if (not para.skip and
                    not para.patterns.get('has_email') and  # NEVER skip email-containing paras
                    (para.patterns.get('has_phone') or para.patterns.get('has_fax') or
                     para.patterns.get('has_tel') or para.patterns.get('has_postal_code'))):
            ...
            if len(text) < 50:
                ...
                if hl == 1:
                    ...
                    # Guard: also preserve paragraphs that carry an email address — these
                    # are FREEFORM contact lines like "Contact Us email@csa.ca" that the
                    # vendor keeps as body content in the FREEFORM section.
                    _is_allcaps_section = (
                        _past_title
                        and para.patterns.get('is_all_caps', False)
                        and len(text.split()) >= 3
                    )
                    # Also preserve URL-only lines (www. or https://) — same reasoning
                    # as email exemption: these are regulatory jurisdiction websites
                    # listed in the document body (e.g. CSA member jurisdictions list).
    ...
    def _filter_global_noise(self, content: List[Dict]) -> List[Dict]:
        ...
        for pi, item in enumerate(para_items):
            ...
            # PatternBasedTagger uses para.patterns['has_phone'] / ['has_email'] / ['has_url']
            # to assign LINE tag (contact-block context). Those are legitimate contact
            # details that vendor includes as <LINE> in the doc body.
            # Feedback #5/#6 fix: include has_url in the guard so bare URL lines are
            # never silently dropped — they are preserved as <LINE> tags.
            if (_PURE_PHONE_RE.match(text)
                    or _PURE_EMAIL_RE.match(text)
                    or _PURE_URL_RE.match(text)):
                _pats = getattr(para, 'patterns', {}) or {}
                _would_be_line = (
                    _pats.get('has_phone', False)
                    or _pats.get('has_email', False)
                    or _pats.get('has_url', False)
                )
    ...
    def _extract_content_in_order(self) -> List[Dict]:
        ...
        for element in body:
            ...
            if tag == 'p':
                ...
                # FIX: use table_total_idx (not data-table count) to fetch the right table.
                # Previously used len([c for c in content if c['type']=='table']) which
                # desynchronised after the first layout table was dropped from content.
                if table_total_idx < len(self.document.tables):
                    table = self.document.tables[table_total_idx]
            ...
            for item in data:
                ...
                for s in item.get("em_spans", []):
                    ...
                    if 0 <= st < en <= len(para.text):
                        # Feedback #3a: snap span boundaries to word edges so <EM>
                        # never splits mid-word.  Move `st` backward to start of
                        # current word, `en` forward to end of current word.
                        _txt = para.text
                        while st > 0 and _txt[st - 1].isalnum():
                            st -= 1
                        while en < len(_txt) and _txt[en].isalnum():
                            en += 1
                        # Strip leading/trailing whitespace from the tagged span
                        while st < en and _txt[st].isspace():
                            st += 1
                        while en > st and _txt[en - 1].isspace():
                            en -= 1
                        if st < en:
...
# ═══════════════════════════════════════════════════════════════════════════════
# DeterministicSGMLFixer
# ───────────────────────
# Applies the SAME deterministic rules as the SGML Validator (L2/L3/L4)
# INSIDE the pipeline — on the LLM-generated SGML string, BEFORE writing to
# disk.  This is NOT post-processing of the final output; it is an integral
# correction pass that guarantees certain properties the validator requires.
#
# Rules sourced from (in order of application):
#   Fix 1  — L2A  : Replace/remove invalid DTD tags (DIV, SPAN, BR, B, I…)
#   Fix 2  — L4D6 : Unicode → Carswell SGML entity mapping in text nodes
#   Fix 3  — L2F  : Remove empty content tags (<P></P>, <ITEM></ITEM>…)
#   Fix 4  — L2B11: Strip leading bullet chars from ITEM content
#   Fix 5  — L2A  : Add required LABEL attribute to <CONTAINR> tags
#   Fix 6  — L2D  : Pad empty <TBLCELL> with &nbsp;
#   Fix 7  — L2G  : Reorder POLIDENT children (N → TI → DATE → CITE)
#   Fix 8  — L2A  : Validate/fix POLIDOC LANG, ADDDATE, MODDATE
#   Fix 9  — D2   : Guaranteed EM tagging for NI/MI numbers, email addresses
#   Fix 10 — L2B  : Detect and repair BLOCK level skips
# ═══════════════════════════════════════════════════════════════════════════════
class DeterministicSGMLFixer:
    """Apply deterministic validator rules to pipeline-generated SGML."""
    ...
    VALID_TAGS: frozenset = frozenset({
        ...
        "PARA", "PARAP", "SPARA", "SPARAP", "MDIV", "MDIV1",
        "DEF", "DEFP", "TERM",
        "TI", "N",
        "P", "P1", "P2", "P3", "P4",
        "ITEM", "LINE",
        "TABLE", "SGMLTBL", "TBL",
        "TBLCDEFS", "TBLCDEF", "TBLHEAD", "TBLBODY", "TBLROWS", "TBLROW",
        "TBLCELL", "TBLNOTE", "TBLNOTES",
        "CL", "CLP", "SCL", "SCLP",
        "STATREF", "STATHIST", "REG", "REFN", "ACTUNDER",
        "APPENDIX", "ARTICLE", "CHAPTER",
        "FORM", "SCHEDDOC", "LEGIDDOC", "MISCLAW", "CONTAINR",
        "BOLD", "EM", "SUP", "SUB", "STRIKE", "ITALIC",
        "GRAPHIC", "LINKTEXT", "EDITNOTE",
        "DATE", "CITE", "FOOTNOTE", "FN", "DF",
    })

    INVALID_TAG_MAP: dict = {
        "DIV":    None,
        "SPAN":   "EM",
        "BR":     None,
        "HR":     None,
    ...
    UNICODE_TO_ENTITY: dict = {
        "\u2018": "&lsquo;", "\u2019": "&rsquo;",
        "\u201c": "&ldquo;", "\u201d": "&rdquo;",
        "\u2013": "&ndash;", "\u2014": "&mdash;",
        "\u00a0": "&nbsp;",
        "\u00e9": "&eacute;", "\u00e8": "&egrave;", "\u00ea": "&ecirc;",
        "\u00e0": "&agrave;", "\u00f4": "&ocirc;",  "\u00c9": "&Eacute;",
        "\u00b0": "&deg;",    "\u00b7": "&middot;", "\u00d7": "&times;",
        "\u00b1": "&plusmn;", "\u00a9": "&copy;",   "\u2022": "&bull;",
        "\u2026": "&hellip;", "\u20ac": "&euro;",   "\u2265": "&ge;",
        "\u2264": "&le;",     "\u00bd": "&frac12;", "\u00bc": "&frac14;",
        "\u00b2": "&sup2;",   "\u00b3": "&sup3;",   "\u00ae": "&reg;",
        "\u2122": "&trade;",
        "\u00e2": "&acirc;",  "\u00e7": "&ccedil;", "\u00ee": "&icirc;",
        "\u00ef": "&iuml;",   "\u00f9": "&ugrave;", "\u00fb": "&ucirc;",
        "\u00fc": "&uuml;",   "\u00c0": "&Agrave;", "\u00c2": "&Acirc;",
        "\u00c7": "&Ccedil;", "\u00c8": "&Egrave;", "\u00ca": "&Ecirc;",
        "\u00ce": "&Icirc;",  "\u00d4": "&Ocirc;",  "\u00d9": "&Ugrave;",
        "\u00db": "&Ucirc;",  "\u00dc": "&Uuml;",   "\u00f3": "&oacute;",
        "\u00fa": "&uacute;", "\u00e1": "&aacute;", "\u00ed": "&iacute;",
        "\u2030": "&permil;", "\u00f7": "&divide;",
    }

    REMOVABLE_EMPTY_TAGS: frozenset = frozenset({
        "P", "P1", "P2", "P3", "P4",
        "ITEM", "LINE", "QUOTE",
        "BOLD", "EM", "SUP", "SUB", "STRIKE",
        "LINKTEXT",
    })

    _ITEM_BULLET_RE = re.compile(
        r'(<ITEM(?:\s[^>]*)?>)\s*'
        r'[\u2022\u00b7\u2013\u2014\u2019\u2018\u25aa\u25ab\u25ba\u25cf*\-]\s+'
    )
    ...
    _EM_SKIP_TAGS: frozenset = frozenset({
        "EM", "BOLD", "TI", "N", "DATE", "CITE", "GRAPHIC",
        "TBLCELL", "TBLHEAD", "POLIDENT", "POLIDOC",
    })

    _FRENCH_TRIGGER_RE = re.compile(
        r'\b(et\b|de\b|du\b|des\b|les\b|dans\b|pour\b|par\b|avec\b|sur\b|'
        r'règlement|instrument|loi\b|article\b|paragraphe|alinéa|'
        r'conformément|disposition|autorité|valeurs\s+mobilières|'
        r'bourse|circulaire|présenté|société|titre|marché)\b',
        re.IGNORECASE,
    )

    _POLIDENT_RE = re.compile(r'(<POLIDENT>)(.*?)(</POLIDENT>)', re.DOTALL)
    ...
    def fix(self, sgml: str, jurisdiction: str = '') -> tuple:
        self._changes = []
        original_len = len(sgml)
        sgml = self._fix_invalid_tags(sgml)
        sgml = self._fix_entities_in_text_nodes(sgml)
        sgml = self._fix_empty_tags(sgml)
        sgml = self._fix_item_bullets(sgml)
        sgml = self._fix_containr_labels(sgml)
        sgml = self._fix_tblcell_empty(sgml)
        ...
        for ch in self._changes[:25]:
            print(f"     \u2714 {ch}")
        if len(self._changes) > 25:
            print(f"     ... ({len(self._changes) - 25} more changes)")
        return sgml, list(self._changes)

    def _fix_invalid_tags(self, sgml: str) -> str:
        for bad_tag, replacement in self.INVALID_TAG_MAP.items():
            if bad_tag not in sgml:
                continue
            before = sgml
            if replacement:
                sgml = re.sub(
                    rf'<{re.escape(bad_tag)}(\s[^>]*)?>',
                    lambda m, r=replacement: f'<{r}{m.group(1) or ""}>',
                    sgml, flags=re.IGNORECASE,
                )
                sgml = re.sub(rf'</{re.escape(bad_tag)}>', f'</{replacement}>', sgml, flags=re.IGNORECASE)
                if sgml != before:
                    self._changes.append(f"Tag replacement: <{bad_tag}> \u2192 <{replacement}>")
            else:
                sgml = re.sub(rf'<{re.escape(bad_tag)}(?:\s[^>]*)?>', '', sgml, flags=re.IGNORECASE)
                sgml = re.sub(rf'</{re.escape(bad_tag)}>', '', sgml, flags=re.IGNORECASE)
                if sgml != before:
                    self._changes.append(f"Removed invalid wrapper tag: <{bad_tag}>")
        return sgml

    def _fix_entities_in_text_nodes(self, sgml: str) -> str:
        parts = re.split(r'(<[^>]+>)', sgml)
        changed = False
        for i, part in enumerate(parts):
            if part.startswith('<'):
                continue
            for char, entity in self.UNICODE_TO_ENTITY.items():
                if char in part:
                    parts[i] = parts[i].replace(char, entity)
                    changed = True
        if changed:
            self._changes.append("Unicode \u2192 entity mapping applied to text nodes (L4 D6)")
        return ''.join(parts)

    def _fix_empty_tags(self, sgml: str) -> str:
        for tag in self.REMOVABLE_EMPTY_TAGS:
            pat = re.compile(rf'<{tag}(?:\s[^>]*)?>\s*</{tag}>')
            before = sgml
            sgml = pat.sub('', sgml)
            if sgml != before:
                self._changes.append(f"Removed empty <{tag}> elements (L2 F)")
        return sgml

    def _fix_item_bullets(self, sgml: str) -> str:
        before = sgml
        sgml = self._ITEM_BULLET_RE.sub(r'\1', sgml)
        if sgml != before:
            self._changes.append("Stripped leading bullet chars from <ITEM> content (L2 B11)")
        return sgml

    def _fix_containr_labels(self, sgml: str) -> str:
        if 'CONTAINR' not in sgml:
            return sgml
        def _add_label(m: re.Match) -> str:
            tag = m.group(0)
            if 'LABEL=' in tag:
                return tag
            self._changes.append("Added LABEL='Appendix' to <CONTAINR> (L2 A: LABEL required)")
            return '<CONTAINR LABEL="Appendix">'
        return re.sub(r'<CONTAINR(?:\s[^>]*)?>', _add_label, sgml)

    def _fix_tblcell_empty(self, sgml: str) -> str:
        if 'TBLCELL' not in sgml:
            return sgml
        pat = re.compile(r'(<TBLCELL(?:\s[^>]*)?>)\s*(</TBLCELL>)')
        before = sgml
        sgml = pat.sub(lambda m: f'{m.group(1)}&nbsp;{m.group(2)}', sgml)
        if sgml != before:
            self._changes.append("Padded empty <TBLCELL> with &nbsp; (L2 D)")
        return sgml

    def _fix_polident_order(self, sgml: str) -> str:
        m = self._POLIDENT_RE.search(sgml)
        if not m:
            return sgml
        inner = m.group(2)
        n_m    = re.search(r'<N>[^<]*</N>', inner)
        ti_m   = re.search(r'<TI>(?:(?!</TI>).)*</TI>', inner, re.DOTALL)
        date_m = re.search(r'<DATE(?:\s[^>]*)?>(?:(?!</DATE>).)*</DATE>', inner, re.DOTALL)
        cite_m = re.search(r'<CITE>(?:(?!</CITE>).)*</CITE>', inner, re.DOTALL)
        def _pos(match):
            return match.start() if match else 9999
        if _pos(n_m) <= _pos(ti_m) <= _pos(date_m) <= _pos(cite_m):
            return sgml
        parts = []
        if n_m:    parts.append(n_m.group(0))
        if ti_m:   parts.append(ti_m.group(0))
        if date_m: parts.append(date_m.group(0))
        if cite_m: parts.append(cite_m.group(0))
        if not parts:
            return sgml
        sgml = sgml[:m.start(2)] + '\n'.join(parts) + sgml[m.end(2):]
        self._changes.append("Reordered POLIDENT children: N \u2192 TI \u2192 DATE \u2192 CITE (L2 G)")
        return sgml

    def _fix_polidoc_attrs(self, sgml: str, jurisdiction: str = '') -> str:
        def _fix_date_attr(m: re.Match) -> str:
            attr_name = m.group(1)
            val = m.group(2)
            fixed = self._normalize_date(val)
            if fixed != val:
                self._changes.append(f"Fixed {attr_name}='{val}' \u2192 '{fixed}' (YYYYMMDD, L2 A)")
            return f'{attr_name}="{fixed}"'
        sgml = re.sub(r'((?:ADDDATE|MODDATE))="([^"]+)"', _fix_date_attr, sgml)
        m_polidoc = re.search(r'<POLIDOC\s([^>]+)>', sgml)
        if m_polidoc:
            attr_str = m_polidoc.group(1)
            m_lang = re.search(r'LANG="([^"]*)"', attr_str)
            current_lang = m_lang.group(1) if m_lang else ''
            if current_lang not in ('EN', 'FR'):
                body_sample = re.sub(r'<[^>]+>', ' ', sgml)[:4000]
                french_hits = len(self._FRENCH_TRIGGER_RE.findall(body_sample))
                is_fr_juri = any(kw in (jurisdiction or '').lower()
                                 for kw in ('quebec', 'montreal', 'montreal-exchange', 'mx', 'bourse'))
                detected = 'FR' if (french_hits > 12 or is_fr_juri) else 'EN'
                new_attrs = re.sub(r'LANG="[^"]*"', f'LANG="{detected}"', attr_str)
                sgml = sgml[:m_polidoc.start(1)] + new_attrs + sgml[m_polidoc.end(1):]
                self._changes.append(f"Fixed invalid LANG='{current_lang}' \u2192 '{detected}' (L2 A)")
            elif current_lang == 'EN':
                is_fr_juri = any(kw in (jurisdiction or '').lower()
                                 for kw in ('quebec', 'montreal-exchange'))
                if is_fr_juri:
                    new_attrs = attr_str.replace('LANG="EN"', 'LANG="FR"')
                    sgml = sgml[:m_polidoc.start(1)] + new_attrs + sgml[m_polidoc.end(1):]
                    self._changes.append(f"Corrected LANG: EN \u2192 FR for '{jurisdiction}' (L2 A)")
        return sgml
    ...
    def _normalize_date(self, date_str: str) -> str:
        ...
        if m:
            mon = self._MONTH_MAP.get(m.group(1).lower())
            if mon:
                return f"{m.group(3)}{mon}{int(m.group(2)):02d}"
        m = re.match(r'(\d{1,2})\s+(\w+)\s+(\d{4})', s, re.IGNORECASE)
        if m:
            mon = self._MONTH_MAP.get(m.group(2).lower())
            if mon:
    ...
    def _apply_em_patterns(self, sgml: str) -> str:
        parts = re.split(r'(<[^>]+>)', sgml)
        in_skip = 0
        em_added = False
        for i, part in enumerate(parts):
            if part.startswith('<'):
                ...
                if tag_m:
                    ...
                    if tag_name in self._EM_SKIP_TAGS:
                        in_skip = max(0, in_skip - 1) if closing else in_skip + 1
            else:
                if in_skip > 0 or not part.strip():
                    continue
                modified = part
                for pat in self._EM_PATTERNS:
                    new = pat.sub(lambda m: f'<EM>{m.group(1)}</EM>', modified)
                    if new != modified:
                        em_added = True
                        modified = new
                parts[i] = modified
...
# ── Singleton instance ────────────────────────────────────────────────────────
_DETERMINISTIC_FIXER = DeterministicSGMLFixer()
print(
    f"\u2705 DeterministicSGMLFixer loaded\n"
    f"   Valid tags: {len(DeterministicSGMLFixer.VALID_TAGS)} | "
    f"Entity mappings: {len(DeterministicSGMLFixer.UNICODE_TO_ENTITY)} | "
    f"EM patterns: {len(DeterministicSGMLFixer._EM_PATTERNS)} | "
    f"Invalid tag subs: {len(DeterministicSGMLFixer.INVALID_TAG_MAP)}"
)
...
class CompletePipeline:
    ...
    def __init__(self):
        self.abbyy = None
        self.force_no_inline = False
        self.preserve_data_tables = True    # Legal regulatory docs always have real multi-col tables
    ...
    @staticmethod
    def _inject_docx_instrtext_hyperlinks(sgml_content: str, docx_path: str) -> str:
        """Fix emails mangled by ABBYY (underscore→space substitution) using the canonical
        addresses stored in DOCX mailto: HYPERLINK field codes (<w:instrText>).
        Also injects DOCX-internal HYPERLINK URLs (tsx.com etc.) that were not reachable
        from PDF annotations (e.g. because PDF used expired tracking URLs).
        """
        import zipfile as _zf, re as _re_it

        _INLINE_TAGS = r'EM|BOLD|STRIKE|B|I|U'
        _TAG_BETWEEN = rf'(?:\s*(?:</?(?:{_INLINE_TAGS})[^>]*>\s*)*)*'
        _TRAILING_CLOSE = rf'(?:\s*</(?:{_INLINE_TAGS})>)*'
        _STD_EMAIL_RE = _re_it.compile(
            r'[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}', _re_it.I
        )
        _STD_URL_RE = _re_it.compile(r'(?:https?://|www\.)[^\s<>"\']{6,}', _re_it.I)
        _NAV_ANCHORS_IT = {
            'home', 'trading', 'francais', 'trading status', 'trading notices',
            'toronto stock exchange', 'tsx venture exchange', 'montreal exchange',
            'contact us', 'contact', 'terms of use', 'privacy policy',
            'accessibility', 'fraud prevention', 'login', 'sign in', 'search',
            'menu', 'tsx', 'tsxv', 'resources', 'notices',
        }
        _SKIP_URL_FRAGS_IT = [
            'facebook.com', 'linkedin.com', 'youtube.com', 'twitter.com',
            '//x.com/', 'instagram.com', '#', 'javascript:',
        ]

        # ── 1. Read DOCX instrText (all XML parts including footnotes) ──────────
        canonical_emails = []   # (email_str,)
        instrtext_urls   = []   # (url_str,)
        try:
            with _zf.ZipFile(str(docx_path)) as _z:
                for _fname in _z.namelist():
                    if not _fname.endswith('.xml'):
                        continue
                    _xml = _z.read(_fname).decode('utf-8', errors='replace')
                    # mailto: hyperlinks
                    for _m in _re_it.finditer(
                        r'<w:instrText[^>]*>\s*HYPERLINK\s+&quot;mailto:([^&"]+)&quot;\s*</w:instrText>',
                        _xml, _re_it.I
                    ):
                        _em = _m.group(1).strip().lower().split('?')[0].rstrip('.;,')
                        if '@' in _em and '.' in _em.split('@')[1]:
                            canonical_emails.append(_em)
                    # https:// hyperlinks (for URL injection)
                    for _m in _re_it.finditer(
                        r'<w:instrText[^>]*>\s*HYPERLINK\s+&quot;(https?://[^&"]+)&quot;\s*</w:instrText>',
                        _xml, _re_it.I
                    ):
                        _url = _m.group(1).strip().rstrip('.;,')
                        if not any(_s in _url for _s in _SKIP_URL_FRAGS_IT):
                            instrtext_urls.append(_url)
        except Exception as _e:
            print(f"   DOCX read failed: {_e}")
            return sgml_content

        result = sgml_content

        # ── 2. Fix mangled emails ─────────────────────────────────────────────
        _raw_emails_found = set(_e.lower() for _e in _STD_EMAIL_RE.findall(result))

        for _email in dict.fromkeys(canonical_emails):   # deduplicate, preserve order
            if _email in _raw_emails_found:
                print(f"   instrText email already present: {_email}")
                continue

            _local, _domain = _email.split('@', 1)
            _parts = _local.split('_')

            # Build raw-SGML regex: word parts separated by optional inline tags/spaces
            if len(_parts) > 1:
                _local_raw = _TAG_BETWEEN.join(_re_it.escape(_p) for _p in _parts)
            else:
                # Single-word local part — may be OCR-fragmented across BOLD tags
                _local_raw = _TAG_BETWEEN.join(_re_it.escape(_c) for _c in _local)

            # Domain: allow optional tags between dotted segments
            _dom_parts = _domain.split('.')
            _domain_raw = (_re_it.escape('.') + _TAG_BETWEEN).join(
                _re_it.escape(_dp) for _dp in _dom_parts
            )

            _raw_re = _re_it.compile(
                rf'(?i){_local_raw}@{_domain_raw}{_TRAILING_CLOSE}',
                _re_it.DOTALL
            )

            _m_raw = _raw_re.search(result)
            if not _m_raw:
                print(f"   instrText email: no match in SGML for: {_email}")
                continue

            # Extend match start backward to consume orphaned opening inline tag
            _mstart = _m_raw.start()
            _open_tag = _re_it.search(
                rf'<(?:{_INLINE_TAGS})[^>]*>\s*$', result[:_mstart], _re_it.I
            )
            if _open_tag:
                _mstart = _open_tag.start()

            result = result[:_mstart] + _email + result[_m_raw.end():]
            _raw_emails_found.add(_email)
            print(f"   Fixed mangled email → {_email}")

        # ── 3. Inject DOCX instrText URLs not already in SGML ────────────────
        ...
        try:
            ...
            _doc_it = _docx_mod.Document(str(docx_path))
            _W_NS_IT = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
            _IT_TAG_EL = f"{{{_W_NS_IT}}}instrText"
            for _para_it in _doc_it.paragraphs:
                _anchor_it = _para_it.text.strip()
                if len(_anchor_it) < 2 or _anchor_it.lower() in _NAV_ANCHORS_IT:
                    continue
                for _instr_el in _para_it._element.findall(f'.//{_IT_TAG_EL}'):
                    _itxt_el = (_instr_el.text or '').strip()
                    _mu_it = _re_it.match(r'HYPERLINK\s+"(https?://[^"]+)"', _itxt_el, _re_it.I)
                    if _mu_it:
                        _pu = _mu_it.group(1).strip().rstrip('.;,')
                        if not any(_s in _pu for _s in _SKIP_URL_FRAGS_IT):
                            _para_url_pairs.append((_anchor_it, _pu))
        except Exception as _de:
            print(f"   DOCX para URL read failed: {_de}")

        # url → first non-nav anchor text from DOCX paragraphs
        _url_anchor_map = {}
        for _anch_it, _pu in _para_url_pairs:
            if _pu not in _url_anchor_map:
                _url_anchor_map[_pu] = _anch_it

        def _html_encode_anchor(_s):
            """Encode chars to their SGML entity forms so anchor matches SGML text."""
            return _s.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')

        _BLOCK_CLOSE_IT = _re_it.compile(
            r'</(P[12]?|LINE|TBLCELL|BLOCK\w*|SEC\w*|ANNEX\w*|PREAMBLE)>',
            _re_it.I
        )
        ...
        def _inject_near(_res, _search, _url_inj):
            ...
            if _p < 0:
                return _res, False
            _mc = _BLOCK_CLOSE_IT.search(_res, _p)
            if not _mc:
                return _res, False
            _ins = _mc.start()
            return _res[:_ins] + f' <EM>[{_url_inj}]</EM>' + _res[_ins:], True

        # ── 3b. Process each unique instrText URL ─────────────────────────────
        _seen_it = set()
        for _url_it in instrtext_urls:
            if _url_it in _seen_it:
                continue
            _seen_it.add(_url_it)

            if _url_present_it(_url_it, _existing_urls):
                continue

            _injected_it = False

            # Try anchor-text injection first (HTML-entity-encoded for SGML match)
            _anchor_raw_it = _url_anchor_map.get(_url_it, '')
            if _anchor_raw_it:
                _anchor_enc_it = _html_encode_anchor(_anchor_raw_it)
                result, _injected_it = _inject_near(result, _anchor_enc_it, _url_it)
                if not _injected_it and _anchor_enc_it != _anchor_raw_it:
                    result, _injected_it = _inject_near(result, _anchor_raw_it, _url_it)
                if _injected_it:
                    _existing_urls.add(_url_it)
                    print(f"   Injected instrText URL (anchor): {_url_it[:60]}")

            if not _injected_it:
                # Domain-substring fallback: try all substrings of domain core (len ≥ 8)
                # e.g. "cdsdeterminationscommittees" → tries "determinations", "committee", etc.
                _dom_it = _re_it.sub(r'^https?://(www\.)?', '', _url_it).split('/')[0]
                _core_it = _dom_it.split('.')[0]  # strip TLD
                # Generate all substrings of length ≥ 8, sorted by decreasing length
                _subs_it = sorted(
                    [_core_it[_si:_sj]
                     for _si in range(len(_core_it))
                     for _sj in range(_si + 8, len(_core_it) + 1)],
                    key=lambda _s: -len(_s)
                )
                for _sub_it in _subs_it:
                    result, _injected_it = _inject_near(result, _sub_it, _url_it)
                    if _injected_it:
                        _existing_urls.add(_url_it)
                        print(f"   Injected instrText URL (substr '{_sub_it[:20]}'): {_url_it[:55]}")
                        break

            if not _injected_it:
                print(f"   instrText URL: no SGML anchor for: {_url_it[:70]}")

        return result

    @staticmethod
    def _inject_pdf_hyperlinks(sgml_content: str, pdf_path: str) -> str:
        """Inject missing hyperlink URLs from PDF annotations as <EM>[url]</EM> after anchor text."""
        import re as _re_hl
        _NAV_ANCHORS = {
            'home', 'trading', 'francais', 'trading status', 'trading notices',
            'toronto stock exchange', 'tsx venture exchange', 'montreal exchange',
            'bourse de montreal', 'contact us', 'contact', 'terms of use',
            'privacy policy', 'accessibility', 'fraud prevention', 'fraud',
            'login', 'sign in', 'search', 'menu', 'tsx', 'tsxv', 'resources',
            'notices', 'advisory notices', 'advisory',
        }
        _SKIP_URI_FRAGS = [
            'facebook.com', 'linkedin.com', 'youtube.com', 'twitter.com',
            '//x.com/', 'instagram.com', 'youtu.be',
        ]
        _TRACKING_DOMAINS = ['click.email.tmx.com', 'click.email.tsx.com']

        def _follow_redirect(uri, timeout=8):
            """Follow HTTP redirects to get final URL.  Uses GET (not HEAD) because
            many tracking systems (TMX click.email.*) don't honour HEAD redirects."""
            try:
                import urllib.request as _urlreq
                import urllib.error as _urlerr
                # Use GET with stream-read to avoid downloading body
                req = _urlreq.Request(uri, method='GET')
                req.add_header('User-Agent', 'Mozilla/5.0 (compatible; SGMLConverter/2.3)')
        ...
        def _strip_tags_with_map(text):
            ...
            while i < len(text):
                if text[i] == '<':
                    ...
                    pos_map.append(i)
                    i += 1
            return ''.join(stripped), pos_map

        try:
            import fitz as _fitz
            doc = _fitz.open(str(pdf_path))
        except Exception as _e:
            print(f"   Unable to open PDF for hyperlink injection: {_e}")
            return sgml_content

        uri_info = {}  # final_uri -> (anchor_text, page_context_str)
        for page in doc:
            ...
            for link in page.get_links():
                ...
                if not uri.startswith(('http://', 'https://')):
                    continue
                if any(s in uri for s in _SKIP_URI_FRAGS):
                    continue
                r      = link['from']
                anchor = _re_hl.sub(r'\s+', ' ', page.get_textbox(r)).strip()
                anchor = _re_hl.sub(r'[\s.,;:)\]]+$', '', anchor).strip()
                if not anchor or len(anchor) < 2:
                    continue
                if any(c in anchor for c in ('\uf230', '\uf0e1', '\uf16a', '\uf179')):
                    continue  # icon / social-media glyphs
                if anchor.lower() in _NAV_ANCHORS:
                    continue
                line_rect = _fitz.Rect(0, r.y0 - 2, page_w, r.y1 + 2)
                page_ctx  = _re_hl.sub(r'\s+', ' ', page.get_textbox(line_rect)).strip()
                final_uri = uri
                if any(domain in uri for domain in _TRACKING_DOMAINS):
                    final_uri = _follow_redirect(uri)
                    if any(domain in final_uri for domain in _TRACKING_DOMAINS):
                        final_uri = uri if not anchor.startswith('http') else anchor.rstrip('.')
                if anchor.startswith(('http://', 'https://')):
                    final_uri = anchor
                prev = uri_info.get(final_uri)
                if not prev or len(anchor) > len(prev[0]):
                    uri_info[final_uri] = (anchor, page_ctx)
        doc.close()

        if not uri_info:
            return sgml_content

        existing_urls = set(_re_hl.findall(
            r'(?:https?://|www\.)[^\s<>"\' \]]+', sgml_content, _re_hl.I
        ))
        result = sgml_content
        ...
        for uri, (anchor, page_ctx) in uri_info.items():
            ...
            if _is_tracking:
                if uri in existing_urls:
                    print(f"   URL already present: {uri[:60]}")
                    continue
            else:
                _pfx = uri[:min(60, len(uri))]
                if any(eu.startswith(_pfx) or _pfx.startswith(eu[:min(60, len(eu))]) for eu in existing_urls):
                    print(f"   URL already present: {uri[:60]}")
                    continue

            stripped, pos_map = _strip_tags_with_map(result)
            pos_s = stripped.find(anchor)
            alt_anchor = None
            if pos_s < 0:
                anchor_alt = anchor.replace('&', '&amp;')
                pos_s = stripped.find(anchor_alt)
                if pos_s >= 0:
                    alt_anchor = anchor_alt

            if pos_s >= 0 and len(anchor) < 15 and stripped.count(anchor) > 1:
                used_anchor = alt_anchor or anchor
                anchor_in_ctx = page_ctx.find(used_anchor)
                if anchor_in_ctx >= 0:
                    ctx_before = page_ctx[max(0, anchor_in_ctx - 30):anchor_in_ctx].lstrip()
                    if ctx_before:
                        search_str = (ctx_before[-20:] if len(ctx_before) > 20 else ctx_before) + used_anchor
                        ctx_pos = stripped.find(search_str)
                        if ctx_pos >= 0 and stripped.count(search_str) == 1:
                            pos_s = ctx_pos + len(search_str) - len(used_anchor)
                        else:
                            print(f"   Ambiguous anchor: '{anchor[:40]}' -> {uri[:50]}")
                            continue
                    else:
                        print(f"   Short anchor, no ctx: '{anchor[:40]}' -> {uri[:50]}")
                        continue
                else:
                    print(f"   Anchor not in page ctx: '{anchor[:40]}' -> {uri[:50]}")
                    continue
            elif pos_s < 0:
                print(f"   Anchor not found: '{anchor[:40]}' -> {uri[:50]}")
                continue

            used_anchor = alt_anchor or anchor
            if pos_s >= len(pos_map):
                continue
            sgml_anchor_start = pos_map[pos_s]
            insert_pos = sgml_anchor_start + len(used_anchor)
            m = _re_hl.match(r'((?:</[A-Z]+>\s*)+)', result[insert_pos:], _re_hl.I)
            if m:
                insert_pos += len(m.group(1))

            result = result[:insert_pos] + f' <EM>[{uri}]</EM>' + result[insert_pos:]
            existing_urls.add(uri)
            print(f"   Injected URL: {uri[:70]}")
    ...
    def convert(self, pdf_path: str) -> Dict[str, Any]:
        ...
        if self.llm_layer and ambiguous:
            ...
            # confirmed BLOCK paragraphs so mis-levelled headings are caught/corrected.
            _block_verify = [p for p in confirmed if p.final_tag.startswith('BLOCK')]
            if _block_verify:
                ...
                print('\n5️⃣ Skipping LLM structural agent (no headings to verify)...')
        else:
            print("\n5️⃣ Skipping LLM structural agent...")
        ...
        # Step 6.5: Deterministic validator rules — applied IN-PIPELINE
        print("\n6️⃣.5 Applying deterministic validator rules (DeterministicSGMLFixer)...")
        try:
            _juri_for_fixer = getattr(self.pattern_tagger, 'jurisdiction', '') or ''
            sgml_content, _fixer_changes = _DETERMINISTIC_FIXER.fix(
                sgml_content, jurisdiction=_juri_for_fixer
            )
        except Exception as _fixer_exc:
            print(f"   ⚠️  DeterministicFixer error (non-fatal): {_fixer_exc}")

        # Step 6.7: Inject PDF hyperlink URLs missing from SGML
        print("\n6.7 Injecting PDF hyperlink URLs...")
        try:
            sgml_content = self._inject_pdf_hyperlinks(sgml_content, pdf_path)
        except Exception as _hl_exc:
            print(f"   Hyperlink injection error (non-fatal): {_hl_exc}")

        # Step 6.8: Fix emails mangled by ABBYY (underscore→space) using instrText canonical form
        #           and inject DOCX instrText URLs (tsx.com etc.) missing from PDF annotations
        print("\n6.8 Fixing DOCX instrText emails & URLs...")
        try:
